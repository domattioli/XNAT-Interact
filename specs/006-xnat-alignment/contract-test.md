...
|---|---|---|---|---|---|
| GAP-001 | Assessor file upload (T003) | `create_assessor(files=[...])` stages files in `_staged_files`; `list_files()` returns them | **RESOLVED** — pyxnat's nested helper (`assessor.resource(label).file(name).put(...)`) constructs `.../experiments/<exp>/assessors/<label>/resources/<N>/files/<f>`, which XNAT 1.9.3 returns HTTP 404 for. Fix: after `assessor.create(xsiType=...)`, resolve the assessor accession ID via `assessor.id()` (falls back to `assessor.attrs.get("ID")`), then PUT via the assessor's direct experiment URI: `PUT /data/experiments/<aid>/resources/<label>/files/<name>` using `Interface._exec(uri, method="PUT", body=<bytes>, ...)`. Proven HTTP 200, file verified on server. T003 dual-run xfails removed; real-side branch now asserts upload succeeds. Fix commit: `fix: GAP-001 — assessor file upload via direct experiment URI (pyxnat nested-path 404 workaround)`. |
