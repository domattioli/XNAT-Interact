...
| 11 | Operational hardening: deploy 009 identity layer (provision `XNAT_IDENTITY_SALT` + encrypt crosswalk at rest + migrate live `ConfigTables` JSON with parity/rollback), Postgres backend path (#34), reproducible `boot-and-verify-local-xnat` | [`011-operational-hardening`](011-operational-hardening/) | 📝 Spec (3 user stories P1–P3; clarify pass pending; localhost + synthetic only) |
