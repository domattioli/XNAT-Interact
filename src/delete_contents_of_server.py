...
def delete_metatables(server: XnatGateway, *, dry_run: bool = False) -> None:
    """Delete the shared config catalog (database_config.json) from the project.
    ...
    """
    # Canonical resource/file names for the shared catalog live in
    # src/utilities.py:93 ('config', 'database_config.json'). The prior
    # 'MetaTables'/'MetaTables.json' constants matched no server resource, so
    # `--method metatables` silently no-opped (#33 M9).
