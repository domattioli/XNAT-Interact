...
class ORDataIntakeForm( ResourceFile ):
    ...
    def _filer_name_and_performance_date_date_user_prompts( self ):
        ...
        if form_available == '1':   self._form_available = True
            ...
            print( f'\n\t**No OR Data Intake Form available for this procedure; assigning EPIC start time as midnight**' )
            self._form_available = False
            midnight = datetime.now().replace(hour=0, minute=0, second=0, microsecond=0)
            self._epic_start_time = midnight.strftime('%Y-%m-%d %H:%M:%S')
    ...
    def _surgical_procedure_info_user_prompts( self ): # Make sure fields that might be stored in the metatables are all completely capitalized
        ...
        print( f'\t(1/29)\tInstitution Name\t--\tPlease Copy-and-Paste from the following list:\t{acceptable_institutions}' )
        ...
        if self.form_is_available:
            ...
            epic_start_time = self.get_time_input( '\t(4/29)\tEpic Start Time (HH:MM):\t' )
    ...
    def _skills_assessment_user_prompts( self ):
        if self.form_is_available:
            ...
            print( f'\t(19/29)\tWas a Skills Assessment requested for this procedure?\n\tEnter "1" for Yes or "2" for No.')
    ...
    def _storage_device_user_prompts( self ):
        ...
        self._relevant_folder = input( '\t(26/29)\tPlease enter the full directory name of the folder containing the case data:\t' ).upper()
    ...
    @property
    def datetime( self )                                -> Opt[USCentralDateTime]:    return USCentralDateTime( f'{self.performance_date} {self.epic_start_time}' )
    @property   
